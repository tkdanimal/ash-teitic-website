// set the starting position of the cursor outside of the screen
			...
			const initCanvas = () => {
				...
				paper.setup(canvas);
				var strokeColor = "rgba(255,196,119)";
				const strokeWidth = 1;
				const segments = 8;
				const radius = 15;
				...
				const noiseRange = 17; // range of distortion
			...
			const initHovers = () => {
				...
				const mainNavItems = document.querySelectorAll(".nav");
				mainNavItems.forEach(item => {
					item.addEventListener("mouseenter", mainNavEnter);
					item.addEventListener("mouseleave", mainNavLeave);
				});
				...
				arrowItems.forEach(item => {
					item.addEventListener("mouseenter", arrowEnter);
					item.addEventListener("mouseleave", arrowLeave);
				});
			};
